...
# ==========================================
# 1. SETUP & STYLE (กลับมาใช้ Style เดิมที่คุณชอบ)
# ==========================================
st.set_page_config(page_title="Beam Insight V6.5", layout="wide", page_icon="🏗️")
...
with st.sidebar:
    st.title("Beam Insight V6.5")
    st.caption("Tab 1-3 Original, Tab 4 Fixed")
    st.divider()
...
# --- TAB 1: BEAM ANALYSIS (คงเดิมตามที่คุณชอบ) ---
with tab1:
...
# --- TAB 4: CALCULATION REPORT (แก้ไขเฉพาะส่วนนี้ ตามที่ขอ) ---
with tab4:
    st.title("📝 Detailed Calculation Report")
    
    st.markdown("#### 1. Section & Material Properties")
    # คงหน้าตา Report Box ไว้ แต่ตัวเลขเป็นค่าคงที่
    st.markdown(f"""
    <div class="report-box">
        <b>Section:</b> {sec_name} <br>
        $A_w = {h_cm:.1f} \\times {tw_cm:.1f} = {Aw:.2f} \\text{{ cm}}^2$ <br>
    ...
    with c_rep1:
        # แก้ไข: แสดงเป็นตัวเลขคูณกันตรงๆ ไม่ใช้ตัวแปร Fy, Aw
        st.latex(rf"V_{{allow}} = 0.4 \times {fy} \times {Aw:.2f} = \mathbf{{{V_cap:,.0f}}} \text{{ kg}}")
    with c_rep2:
        # แก้ไข: แสดงเป็นตัวเลขคูณกันตรงๆ ไม่ใช้ตัวแปร Fy, Zx
        st.latex(rf"M_{{allow}} = 0.6 \times {fy} \times {Zx} = \mathbf{{{M_cap:,.0f}}} \text{{ kg.cm}}")
    
    st.markdown("#### 3. Connection Design")
    st.markdown(f"""
    <div class="report-box">
        <b>Bolt Capacity ({bolt_size}):</b> <br>
        1. Shear: $1000 \\times {b_area} = {v_bolt_shear:,.0f}$ kg <br>
        2. Bearing: $1.2 \\times 4000 \\times {dia_cm} \\times {tw_cm} = {v_bolt_bear:,.0f}$ kg <br>
