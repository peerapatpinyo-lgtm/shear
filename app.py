...
with tab1:
    ...
    def render_check_box(title, act, lim, ratio_label, eq_w, eq_act, eq_ratio):
        ratio = act / lim
        is_pass = ratio <= 1.01 
    ...
    with c1:
        render_check_box("Shear Check", v_act, V_cap, "V/V_cap",
            fr"w_{{limit}} = {w_shear:,.0f} \text{{ kg/m}}",
            fr"V_{{act}} = {v_act:,.0f} \text{{ kg}}",
            fr"Ratio = {v_act/V_cap:.3f}")
    with c2:
        render_check_box("Moment Check", m_act, (M_cap/100), "M/M_cap",
            fr"w_{{limit}} = {w_moment:,.0f} \text{{ kg/m}}",
            fr"M_{{act}} = {m_act:,.0f} \text{{ kg.m}}",
            fr"Ratio = {m_act/(M_cap/100):.3f}")
    with c3:
        render_check_box("Deflection Check", d_act, d_all, "Δ/Δ_all",
            fr"w_{{limit}} = {w_defl:,.0f} \text{{ kg/m}}",
            fr"\Delta_{{act}} = {d_act:.3f} \text{{ cm}}",
            fr"Ratio = {d_act/d_all:.3f}")
    ...
    fig.add_trace(go.Scatter(x=[user_span], y=[user_safe_load], mode='markers+text', name='Design Point', text=[f"  ({user_span}m, {user_safe_load:,.0f}kg/m)"], marker=dict(color='red', size=12)))
    st.plotly_chart(fig, use_container_width=True)

with tab2:
    try:
        req_bolt, v_bolt = conn.render_connection_tab(
            V_design=V_design, bolt_size=bolt_size, method=method, 
            is_lrfd=is_lrfd, section_data=p, conn_type=conn_type, bolt_grade=bolt_grade
        )
...
with tab4:
    res = {'w_safe': user_safe_load, 'cause': user_cause, 'v_cap': V_cap, 'v_act': v_act, 'm_cap': M_cap, 'm_act': m_act, 'd_all': d_all, 'd_act': d_act}
    bolt_info = {'size': bolt_size, 'qty': req_bolt if 'req_bolt' in locals() else 0, 'type': conn_type, 'grade': bolt_grade}
    rep.render_report_tab(method, is_lrfd, sec_name, grade_choice, p, res, bolt_info)
