...
# ==========================================
# 1. SETUP & STYLE (PREMIUM UI V11 Base)
# ==========================================
...
<style>
    ...
    /* Highlight Card with Dynamic Color Bar */
    .highlight-card { 
...
# ==========================================
# 3. CORE CALCULATION (ASD vs LRFD Logic)
# ==========================================
p = steel_db[sec_name]
...
if is_lrfd:
    # LRFD Constants
    phi_b = 0.90
    phi_v = 1.00 # For rolled I-shapes
    
    # Capacity (Factored Strength)
    M_cap = phi_b * fy * Zx        # 0.9 Fy Zx
    V_cap = phi_v * 0.6 * fy * Aw  # 0.6 Fy Aw
    
    label_load = "Factored Load (Wu)"
    label_cap_m = "Phi Mn"
    label_cap_v = "Phi Vn"
    bolt_factor = 1.5 # Approximation: LRFD bolt cap is ~1.5x ASD
else:
    # ASD Constants
    M_cap = 0.6 * fy * Zx
    V_cap = 0.4 * fy * Aw
...
# Bolt Capacity Base (ASD) -> Adjusted if LRFD
...
def get_capacity(L_m):
    ...
    # Deflection is Service Load. In LRFD context, comparison is tricky. 
    # For this App: We calculate w that causes limit deflection.
...
with tab1:
    ...
    # 2. Logic Source (Dynamic Text based on Method)
...
# TAB 2 & 3 Similar style but updated values...
with tab2:
