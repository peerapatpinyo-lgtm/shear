# drawing_utils.py (RESTORED ORIGINAL FIN PLATE + ADDED NEW TYPES)
...
# =============================================================================
# 🛠️ HELPER FUNCTIONS (ใช้แบบ Pixel-Fixed เพื่อป้องกันการ Zoom ผิดปกติ)
# =============================================================================
def add_dim(fig, x0, y0, x1, y1, text, offset=30, type="h", color=STYLE['DIM']['color'], is_err=False):
    c = "#EF4444" if is_err else color
    if type == "h": 
        y_pos = y0 + offset
...
def draw_h_beam_section(fig, x_cen, y_cen, h, b, tf, tw, style, orientation="I"):
    if orientation == "I":
        ...
        fig.add_shape(type="rect", x0=x_cen+h/2-tf, y0=y-cen-b/2, x1=x_cen+h/2, y1=y_cen+b/2, fillcolor=style['fillcolor'], line=style['line'])
...
def create_plan_view(beam, plate, bolts):
    ...
    # ----------- NEW LOGIC (END PLATE / DOUBLE ANGLE) -----------
    elif "End" in ctype:
        # End Plate Plan
        ...
        fig.add_shape(type="rect", x0=bx-d, y0=full_t/2+15, x1=bx+d, y1=full_t/2+25, fillcolor="black", line=dict(width=0))
    ...
    if "Fin" in ctype:
        ...
        for c in range(cols):
            ...
            for r in range(rows):
                ...
                fig.add_shape(type="line", x0=cx-d, y0=cy, x1=cx+d, y1=cy, line=dict(color="black", width=0.5))
                fig.add_shape(type="line", x0=cx, y0=cy-d, x1=cx, y1=cy+d, line=dict(color="black", width=0.5))
                bolt_count += 1
...
def create_side_view(beam, plate, bolts):
    ...
    if "Fin" in ctype:
        ...
        add_leader(fig, -tw/2, h/2-tf, f"Beam H{h}", ax=-60, ay=40)
