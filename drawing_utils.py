...
# =============================================================================
# 🎨 COLOR PALETTE & STYLES (คงไว้ตามมาตรฐานที่คุณกำหนด)
# =============================================================================
...
# =============================================================================
# 🛠️ HELPER TOOLS (ฟังก์ชันมาตรฐาน CAD ของคุณ)
# =============================================================================
def add_cad_dim(fig, x0, y0, x1, y1, text, type="horiz", offset=0):
    """ เส้นบอกระยะ (Dimension Line) - มาตรฐาน CAD """
    arrow_head_style = 2  
    arrow_scale = 1.0     
    arrow_width = 0.8     
...
# =============================================================================
# 1. PLAN VIEW (TOP) - กลับมาครบถ้วนเพื่อแก้ Error
# =============================================================================
def create_plan_view(beam, plate, bolts):
    ...
    fig.update_layout(title="<b>PLAN VIEW</b>", plot_bgcolor="white", margin=dict(l=5,r=5,t=30,b=30), height=300,
                      ...
                      yaxis=dict(visible=False, range=[-zoom_y-30, zoom_y], scaleanchor="x", scaleratio=1, fixedrange=True))
    return fig

# =============================================================================
# 2. FRONT VIEW (ELEVATION) - กลับมาครบถ้วน
# =============================================================================
def create_front_view(beam, plate, bolts):
    ...
    fig.update_layout(title="<b>ELEVATION VIEW</b>", plot_bgcolor="white", margin=dict(l=5,r=35,t=30,b=30), height=320,
                      xaxis=dict(visible=False, range=[zoom_x_start, zoom_x_end], fixedrange=True),
                      yaxis=dict(visible=False, range=[-zoom_y_limit-30, zoom_y_limit], scaleanchor="x", scaleratio=1, fixedrange=True))
    return fig

# =============================================================================
# 3. SIDE VIEW (SECTION) - แก้ไขให้ถูกต้องตามหลักวิศวกรรม
# =============================================================================
def create_side_view(beam, plate, bolts):
    ...
    # --- วาดหน้าตัดเสา (Column I-Section) ขวางอยู่ทางซ้าย ---
    # ตามรูป Sketch สีแดงของคุณที่ต้องการให้เห็นหน้าตัดเสาจริง
    col_x_offset = -b/2 - 40
    # ปีกเสา (Top & Bottom Flanges)
    fig.add_shape(type="rect", x0=col_x_offset-b/2, y0=h/2+10, x1=col_x_offset+b/2, y1=h/2+10+tf, line=dict(color=C_BEAM_OUT, width=1.5), fillcolor=C_COL_FILL)
    fig.add_shape(type="rect", x0=col_x_offset-b/2, y0=-h/2-10-tf, x1=col_x_offset+b/2, y1=-h/2-10, line=dict(color=C_BEAM_OUT, width=1.5), fillcolor=C_COL_FILL)
    # เอวเสา (Web)
    fig.add_shape(type="rect", x0=col_x_offset-tw/2, y0=-h/2-10, x1=col_x_offset+tw/2, y1=h/2+10, line=dict(color=C_BEAM_OUT, width=1.5), fillcolor=C_COL_FILL)

    # --- วาดหน้าตัดคาน (Beam I-Section) ---
    ...
    # วาดน็อต Bolt (ด้านข้าง)
    by_start = h_pl/2 - lv
    bolt_x = tw/2 + t_pl
    for r in range(n_rows):
        by = by_start - r*s_v
        fig.add_shape(type="rect", x0=bolt_x, y0=by-d_bolt/2, x1=bolt_x+10, y1=by+d_bolt/2, fillcolor=C_BOLT_FILL, line_width=1)

    # --- Dimensions ---
    add_cad_dim(fig, -b/2, h/2, b/2, h/2, f"B={b:.0f}", offset=30)
    add_cad_dim(fig, -b/2-20, h/2, -b/2-20, -h/2, f"H={h:.0f}", "vert")

    fig.update_layout(title="<b>SIDE VIEW (SECTION)</b>", plot_bgcolor="white", margin=dict(l=5,r=5,t=30,b=5), height=300,
                      xaxis=dict(visible=False, range=[col_x_offset-b, b+100], fixedrange=True),
