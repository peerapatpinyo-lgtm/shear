# drawing_utils.py (V17 - Fixed Style Definition)
import plotly.graph_objects as go
import numpy as np

# =============================================================================
# 🎨 CAD STANDARD STYLES (Fixed)
# =============================================================================
STYLE = {
    # สำหรับ Shape ที่มีทั้ง Fill และ Line
    'STEEL_CUT':   dict(fillcolor="#E5E7EB", line=dict(color="#1F2937", width=2)), # ตัดเหล็ก
    'STEEL_FACE':  dict(fillcolor="#F9FAFB", line=dict(color="#9CA3AF", width=1)), # ผิวเหล็ก
    'PLATE':       dict(fillcolor="#DBEAFE", line=dict(color="#1E40AF", width=2)), # Plate
    'BOLT':        dict(fillcolor="#4B5563", line=dict(color="#111827", width=1.5)), # Bolt
    
    # สำหรับเส้นเปล่าๆ (Line Only) - แก้ไขตรงนี้ไม่ให้ซ้อน dict
    'WELD':        dict(fillcolor="#000000", line=dict(width=0)),
    'CL':          dict(color="#DC2626", width=1, dash="dashdot"),      # Center Line
    'DIM':         dict(color="#1D4ED8", family="Sarabun", size=12),    # Dimension Color
    'ERROR':       dict(color="#EF4444", width=2, dash="dash")          # Error Line (แก้แล้ว: เอา line=dict() ออก)
}
...
def create_front_view(beam, plate, bolts):
    ...
    # Objects
    fig.add_shape(type="line", x0=0, y0=-h_b/2-100, x1=0, y1=h_b/2+100, line=dict(color="black", width=3)) # Col Line
    fig.add_shape(type="rect", x0=SETBACK, y0=-h_b/2, x1=w_pl+150, y1=h_b/2, 
                  fillcolor=STYLE['STEEL_FACE']['fillcolor'], line=dict(color="#E5E7EB", width=1)) # Beam Ghost
    fig.add_shape(type="rect", x0=0, y0=y_bot, x1=w_pl, y1=y_top, 
                  fillcolor=STYLE['PLATE']['fillcolor'], line=STYLE['PLATE']['line']) # Plate

    if is_short:
        user_y_bot = y_top - h_pl_input
        # ตรงนี้คือจุดที่เคย Error ตอนนี้ใช้ STYLE['ERROR'] ที่แก้แล้ว
...
def create_side_view(beam, plate, bolts):
    ...
    for r in range(rows):
        ...
        fig.add_shape(type="rect", x0=-tw/2-10, y0=y-bolts['d']/2, x1=tw/2+tp, y1=y+bolts['d']/2,
                      fillcolor=STYLE['BOLT']['fillcolor'], line=dict(width=0))
        fig.add_shape(type="rect", x0=tw/2+tp, y0=y-bolts['d']/2, x1=tw/2+tp+15, y1=y+bolts['d']/2, # Head
                      fillcolor="black", line=dict(width=0))

    # Dims
    add_dim(fig, -b/2-20, h/2, -b/2-20, -h/2, f"Beam={h}", 30, "v")
    add_dim(fig, b/2+30, h_pl/2, b/2+30, -h_pl/2, f"Plate={h_pl}", 30, "v")
