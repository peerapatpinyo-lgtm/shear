...
def generate_report(V_load, beam, plate, bolts, is_lrfd=True, material_grade="A36", bolt_grade="A325"):
    ...
    def calc_block(symbol, description, formula_tex, sub_tex, result, unit):
        # เทคนิค: ใช้ \n$$ เพื่อบังคับขึ้นบรรทัดใหม่สำหรับสมการเสมอ
        # และใช้ raw string (r"") ในบางจุดถ้าทำได้ แต่ตรงนี้ใช้ f-string แบบระวัง backslash
        return f"""
**{description}**
...
"""
    ...
    def ratio_bar(demand, capacity, label="Ratio"):
        ...
        # HTML Block
        return f"""
<div style="margin-top: 5px; margin-bottom: 15px; padding: 10px; background-color: #f0f2f6; border-radius: 5px;">
  <strong>Check {label}:</strong> {demand:.2f} / {capacity:.2f} = <strong>{ratio:.2f}</strong> &nbsp; 
...
""")
    ...
    Ag = h_pl * t_pl
    Rn_yld = (0.60 * Fy_pl * Ag) / 1000.0; cap_yld = Rn_yld * get_f('y')
    ...
    Rn_rup = (0.60 * Fu_pl * An) / 1000.0; cap_rup = Rn_rup * get_f('r')
    ...
    Rn_blk = min((0.6 * Fu_pl * Anv + 1.0 * Fu_pl * Ant), (0.6 * Fy_pl * Agv + 1.0 * Fu_pl * Ant)) / 1000.0
    ...
    L_weld = h_pl * 2
    Rn_weld = (0.60 * 480 * 0.707 * w_sz * L_weld) / 1000.0; cap_weld = Rn_weld * get_f('w')
