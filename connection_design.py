...
import math
import drawing_utils as draw # <--- Import ไฟล์ Drawing ที่เราแยกไว้
...
def render_connection_tab(V_design_from_tab1, default_bolt_size, method, is_lrfd, section_data, conn_type, default_bolt_grade, default_mat_grade):
    ...
    with col_result:
        ...
        # DRAWING SECTION (Calling external util)
        st.markdown("#### 📐 Engineering Drawing")
        
        with st.spinner("Generating CAD Drawings..."):
            # Prepare data dicts for drawing function
            plate_dims = {'h': h_plate, 'w': 15.0, 't': pl_thick}
            bolt_dims = {'rows': n_rows, 'cols': n_cols, 'pitch': pitch, 'edge': edge_dist, 'dia': d_bolt}
            
            # Call the function from drawing_utils.py
            fig_cad = draw.draw_fin_plate_3views(section_data, plate_dims, bolt_dims, weld_sz)
            
            st.pyplot(fig_cad)
        
        st.caption("Note: Visualization generated by `drawing_utils.py`")
