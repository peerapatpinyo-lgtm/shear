...
def render_connection_tab(V_design, bolt_size, method, is_lrfd, section_data, conn_type, bolt_grade, T_design=0):
    ...
    # ==========================================
    # TAB 2: SHOP DRAWING (ROTATED 90 DEG)
    # ==========================================
    with tab2:
        ...
        # =========================================================
        # Concept:
        # X-Axis = Width (Across the Beam Flange)
        # Y-Axis = Length (Along the Beam Axis) -> Beam points UP
        # =========================================================
        with col_sec:
            ...
            # Draw a wide rectangle representing the Column Flange face
            ...
            fig_sec.add_shape(type="rect", 
                              x0=web_x0, y0=10, # Start with gap 
            ...
            # Plate runs ALONG Y-axis (Length) and has Thickness in X-axis
            if plate_side == "Right of Web":
                px0, px1 = web_x1, web_x1 + t_plate_mm
            else:
                px0, px1 = web_x0 - t_plate_mm, web_x0
            
            py0, py1 = 0, plate_w # Attached to Col Face (0) to Plate Width
            ...
            # 4. BOLTS
            # Bolt spacing corresponds to Y-axis (Horiz Pitch sh)
            bolt_start_y = e1_mm
            
            for c in range(n_cols):
                by = bolt_start_y + c*s_h # Y coordinate is distance from col
                
                # Bolt Shank (Horizontal line across Web+Plate)
                sx0 = min(px0, web_x0)
                sx1 = max(px1, web_x1)
        ...
        # =========================================================
        # VIEW 2: ELEVATION (SIDE VIEW - STANDARD)
        # =========================================================
        with col_elev:
            st.markdown("**ELEVATION (Side)**")
            fig_elev = go.Figure()
            
            # Coordinates: X = Along Beam, Y = Vertical Height
            ...
            # 3. Plate (Vertical Rectangle) - "Plate must be vertical"
