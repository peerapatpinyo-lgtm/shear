...
def render_report_tab(method, is_lrfd, sec_name, steel_grade, p, res, bolt):
    ...
    # คำนวณ Ratio แยกไว้ข้างนอกเพื่อความชัดเจนและลดโอกาสเกิด Syntax Error ใน f-string
    shear_ratio = v_act / v_cap if v_cap != 0 else 0
    moment_ratio = m_act / (m_cap / 100) if m_cap != 0 else 0
    deflection_ratio = d_act / d_all if d_all != 0 else 0
...
<tbody>
<tr><td style="padding:10px;border:1px solid #e5e7eb;text-align:left;">Shear Strength</td><td style="padding:10px;border:1px solid #e5e7eb;">{v_act:,.0f} kg</td><td style="padding:10px;border:1px solid #e5e7eb;">{v_cap:,.0f} kg</td><td style="padding:10px;border:1px solid #e5e7eb;font-weight:bold;">{shear_ratio:.3f}</td></tr>
<tr><td style="padding:10px;border:1px solid #e5e7eb;text-align:left;">Moment Resistance</td><td style="padding:10px;border:1px solid #e5e7eb;">{m_act:,.0f} kg.m</td><td style="padding:10px;border:1px solid #e5e7eb;">{m_cap/100:,.0f} kg.m</td><td style="padding:10px;border:1px solid #e5e7eb;font-weight:bold;">{moment_ratio:.3f}</td></tr>
<tr><td style="padding:10px;border:1px solid #e5e7eb;text-align:left;">Deflection</td><td style="padding:10px;border:1px solid #e5e7eb;">{d_act:.3f} cm</td><td style="padding:10px;border:1px solid #e5e7eb;">{d_all:.3f} cm</td><td style="padding:10px;border:1px solid #e5e7eb;font-weight:bold;">{deflection_ratio:.3f}</td></tr>
</tbody></table>
