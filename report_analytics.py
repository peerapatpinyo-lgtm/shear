# report_analytics.py
# Version: 23.0 (Fixed: Restored Vertical Lines in Graph & Full 6-Mode Check)
...
def render_analytics_section(load_pct, bolt_dia, load_case, factor):
    ...
    for sec in all_sections:
        ...
        # Note: Derived from 5wL^4/384EI = L/360
        ...
        # 1.3 AUTO-DESIGN (Condensed for brevity, same logic as V22)
    ...
    # Retrieve Limits
    V_allow = row['V_Beam']
    # Back-calculate M_allow and K for plotting curves
    # L_Start = 4 * M / V  => M = L_Start * V / 4
    L_start = row['L_Start']
    L_end = row['L_End']
    
    # Re-derive constants for plotting curves
    M_derived = (L_start * V_allow) / 4 if L_start > 0 else 0
    # L_End = K / 8M => K = L_End * 8M
    ...
    # Curve Formulas
    ws = (2 * V_allow) / spans # Shear Limit (Uniform Load)
    ...
    # Fill Moment Zone
    if L_end > L_start:
        idx_s = np.searchsorted(spans, L_start)
        idx_e = np.searchsorted(spans, L_end)
        ax.fill_between(spans[idx_s:idx_e], 0, w_safe[idx_s:idx_e], color='#E74C3C', alpha=0.15)
        
        # Add Zone Label in the middle
        mid_point = (L_start + L_end) / 2
        safe_load_at_mid = np.interp(mid_point, spans, w_safe)
        ax.text(mid_point, safe_load_at_mid/2, "MOMENT\nZONE", 
