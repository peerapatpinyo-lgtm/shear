...
def render(res_ctx, v_design):
    ...
    with st.container(border=True):
        st.markdown("##### 📐 Construction Specification")
        c1, c2, c3, c4 = st.columns([1.5, 1, 1, 1])
        with c1:
            sec_list = steel_db.get_section_list()
            col_name = st.selectbox("Select Column Size", sec_list, index=sec_list.index(res_ctx['sec_name']) if res_ctx['sec_name'] in sec_list else 0)
    ...
    px, py = 350, 320   # Plan Position
    ax, ay = 350, 850   # Section A-A (Looking along X)
    bx, by = 900, 320   # Section B-B (Looking along Y)

    def draw_tick(x, y): return f'<line x1="{x-4}" y1="{y+4}" x2="{x+4}" y2="{y-4}" stroke="black" stroke-width="1.2"/>'
    ...
    <svg width="{cv_w}" height="{cv_h}" viewBox="0 0 {cv_w} {cv_h}" xmlns="http://www.w3.org/2000/svg">
        ...
        <g transform="translate({px}, {py})">
            <text x="0" y="{-N*sc/2 - 70}" text-anchor="middle" font-weight="bold" font-size="18">PLAN VIEW (TOP)</text>
            ...
            <g transform="translate(0, {N*sc/2 + 50})" font-family="monospace" font-size="11">
                ...
                {draw_tick(-B*sc/2,0)} {draw_tick(-sx*sc/2,0)} {draw_tick(-cb*sc/2,0)} {draw_tick(cb*sc/2,0)} {draw_tick(sx*sc/2,0)} {draw_tick(B*sc/2,0)}
                ...
                <text x="{cb/2*sc + clr_x*sc/2}" y="15" text-anchor="middle" fill="red">{int(clr_x)}</text>
                <text x="{sx/2*sc + edge_x*sc/2}" y="15" text-anchor="middle" fill="green">{int(edge_x)}</text>
                <text x="0" y="45" text-anchor="middle" font-size="14" font-weight="bold">WIDTH B = {int(B)}</text>
            </g>
        </g>

        <g transform="translate({ax}, {ay})">
            <text x="0" y="-140" text-anchor="middle" font-weight="bold" font-size="16">SECTION A-A (LOOKING AT FLANGE)</text>
            ...
            <line x1="{-ctw/2*sc}" y1="-120" x2="{-ctw/2*sc}" y2="0" stroke="#94a3b8" stroke-dasharray="4,2"/>
            <line x1="{ctw/2*sc}" y1="-120" x2="{ctw/2*sc}" y2="0" stroke="#94a3b8" stroke-dasharray="4,2"/>
            <line x1="{-sx/2*sc}" y1="-15" x2="{-sx/2*sc}" y2="150" stroke="#2563eb" stroke-width="3"/>
            <line x1="{sx/2*sc}" y1="-15" x2="{sx/2*sc}" y2="150" stroke="#2563eb" stroke-width="3"/>
        </g>

        <g transform="translate({bx}, {by})">
            <text x="0" y="{-N*sc/2 - 70}" text-anchor="middle" font-weight="bold" font-size="16">SECTION B-B (SIDE SECTION)</text>
            ...
            <rect x="{-ch/2*sc}" y="-120" width="{ctf*sc}" height="120" fill="#94a3b8" stroke="black"/> <rect x="{(ch/2 - ctf)*sc}" y="-120" width="{ctf*sc}" height="120" fill="#94a3b8" stroke="black"/> <rect x="{-ch/2*sc + ctf*sc}" y="-120" width="{(ch - 2*ctf)*sc}" height="120" fill="#cbd5e1" stroke="black" stroke-opacity="0.5"/> <line x1="{-sy/2*sc}" y1="-15" x2="{-sy/2*sc}" y2="150" stroke="#2563eb" stroke-width="3"/>
            <line x1="{sy/2*sc}" y1="-15" x2="{sy/2*sc}" y2="150" stroke="#2563eb" stroke-width="3"/>

            <g transform="translate({N*sc/2 + 50}, 0)" font-family="monospace" font-size="11">
                <line x1="0" y1="{-N*sc/2}" x2="0" y2="{N*sc/2}" stroke="black"/>
                {draw_tick(0,-N*sc/2)} {draw_tick(0,-sy*sc/2)} {draw_tick(0,-(ch/2-ctf)*sc)} {draw_tick(0,(ch/2-ctf)*sc)} {draw_tick(0,sy*sc/2)} {draw_tick(0,N*sc/2)}
                ...
                <text x="60" y="0" transform="rotate(90, 60, 0)" text-anchor="middle" font-size="14" font-weight="bold">LENGTH N = {int(N)}</text>
            </g>
        </g>
    </svg>
    """
